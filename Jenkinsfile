        ...
        stage('Build, Push, Deploy Apps in Parallel') {
            steps {
                script {
                    ...
                    // Loop through apps to create parallel stages
                    for (app in env.APPS) {
                        def appCopy = app  // necessary for closure
                        branches[app.name] = {
                            ...
                            stage("Build Docker ${appCopy.name}") {
                                sh """
                                    docker build --no-cache \
                                        --build-arg APP_NAME=${appCopy.name} \
                                        --build-arg APP_TYPE=${appCopy.type} \
                                        -t ${appCopy.docker_image}:latest \
                                        -f Dockerfile ./${appCopy.name}
                                """
                            }
                            ...
                            stage("Push Docker ${appCopy.name}") {
                                withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                                    sh """
                                        ...
                                        docker push ${appCopy.docker_image}:${BUILD_NUMBER}
                                    """
                                }
                            }
